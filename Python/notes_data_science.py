...
np.set_printoptions(precision = 2, suppress = True)     ----> precicion = 2     ---> 4.33121231     gets converted to   -----> 4.33
                                                        ----> suppress = True   ---> 5.434049e-04   gets converted to   -----> 0.000543
...
--------------------->  5. Array Concatenation (np.concatenate())    and    Splitting (np.split())
...
For MultiDimensinal Array:
    ...
    np.split(arr1, [4,6,7,9], axis = 1)     ----> Splits the Matrix at the 4th, 6th, 7th and 9th Rows, Horizontally
...
For every 'for/while loop' in a Python script, you should consider whether it can be replaced with a vectorized expression.
...
np.sum()          np.nansum()               Compute sum of elements           Eg. np.nansum() ---> will ignore 'nan' values for sum
...
np.any(arr == 8, axis = 0/1)     N/A        Evaluate whether any one of the elements are True
